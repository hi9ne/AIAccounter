...
[💼 Бизнес]
   Доходы от клиентов, расходы на бизнес, фриланс, ИП
```
...
| Бизнес | `business` | Офис, Оборудование, Реклама, Зарплаты сотрудникам, Налоги, Транспорт, Связь, Подписки, Закупки, Другое | Продажи, Услуги, Консалтинг, Комиссии, Другое |
...
| `usage_type` | varchar | Тип (personal/business) |
...
VALUES ({user_id}, '{YYYY-MM}', {amount}, '{currency}')
ON CONFLICT (user_id, month) DO UPDATE SET budget_amount = {amount};
```
