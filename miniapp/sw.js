...
const CACHE_NAME = 'aiaccounter-v5.0.3';
const RUNTIME_CACHE = 'aiaccounter-runtime-v5.0.3';
...
self.addEventListener('install', (event) => {
    console.log('[SW] Installing v5.0.3...');
...
self.addEventListener('activate', (event) => {
    console.log('[SW] Activating v5.0.3...');
    ...
    event.waitUntil(
        caches.keys()
            .then(cacheNames => {
                return Promise.all(
                    cacheNames
                        .filter(name => !name.includes('v5.0.3'))
                        .map(name => {
